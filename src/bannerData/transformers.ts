...
/* 注意：有一个名为 /\p{RGI_Emoji}/v 的新正则表达式，可以处理由多个表情符号组成的表情符号，但 `v` 标志尚不兼容 */
const EMOJI_REGEX =
    /[\p{Extended_Pictographic}\u{1F3FB}-\u{1F3FF}\u{1F9B0}-\u{1F9B3}]+/gu;
const HEADER_KEY_REGEX = /{{(.+?)}}/g;

export const extractIconFromYaml = (
    ...
    return match?.length
        ...
        : { type: "text", value: value.replace("[[", "").replace("]]", "") };
    ...
    value.replace(HEADER_KEY_REGEX, (match, keys: string) => {
        ...
        for (const key of keys.split(",")) {
            ...
            if (trimmedKey === FILENAME_KEY) {
                ...
                const property =
                    plug.app.metadataCache.getFileCache(file)?.frontmatter?.[
                        trimmedKey
                    ];
