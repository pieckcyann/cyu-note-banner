...
import { editorInfoField, Notice, parseYaml } from "obsidian";
...
export const updateBannerData = async (
    file: TFile,
    bannerData: Partial<BannerDataWrite>
) => {
    await plug.app.fileManager.processFrontMatter(file, async (frontmatter) => {
        for (const [dataKey, val] of Object.entries(bannerData) as [
            keyof BannerData,
            any
        ][]) {
...
// 帮助将横幅的源旧语法更新为当前语法
export const updateLegacyBannerSource = async (
    file: TFile
): Promise<boolean> => {
    let changed = false;
