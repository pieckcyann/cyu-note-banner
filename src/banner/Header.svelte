<script lang="ts">
    ...
    // $: {
    //     if (icon) console.log("icon: " + icon.value);
    // }
...
>
    {#if icon}
        <Icon {icon} {isEmbed} {file} on:open-icon-modal />
    {/if}
    {#if header}
...
<style lang="scss">
    .banner-header {
        ...
        &.shadow {
            text-shadow: var(--background-primary) 0 0 6px;
        }
        &.border {
        ...
        &.h-custom {
            transform: var(--banners-header-transform);
        }
